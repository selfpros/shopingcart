...
$(function() {
	...
	/*
	 * var products = [ [ '1', 'xyz' ], [ '2', 'ABC' ], [ '3', 'opq' ], [ '4',
	 * 'RST' ],[ '5', 'kml' ],[ '6', 'MLR' ],[ '7', 'OPQ' ],[ '8', 'wxy' ] ];
	 */
	...
	$(document)
			.on(
					...
					function() {
						alert("sdfcsd");
						var dText=(this.checked)?'you want to activate?':'you want to deactivate?';
						var checked=this.checked;
						var checkbox=$(this);
						alert("check box"+checkbox.html());
						alert("is it working"+dText);
						debugger;
					    bootbox.confirm({
					    	size: 'medium',
					    	title: 'Product Activation/Deactivation',
					...
					columns : [
...

							{
								...
								mRender : function(data, type, row) {
									...
									if (data) {
										str += '<input type="checkbox" class="abc" checked="checked" value="'
												+ row.id + '"/>'

									} else {
										str += '<input type="checkbox" class="abc"  value="'
												+ row.id + '"/>'
